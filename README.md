...
- `admin` というユーザー名のアカウント（通常はスーパーユーザー）はチームメンバーや担当者候補から除外されるため、`管理者` グループに追加する場合は別途一般用アカウントを用意することを推奨する。
...
| チーム管理 | チーム作成・メンバー招待（作成は管理者グループ所属者のみ、メンバー招待は admin ユーザーを除いた全ユーザーから選択可） |
...
| GET | `/users/` | 全ユーザー一覧（`admin` ユーザーを除外） |
